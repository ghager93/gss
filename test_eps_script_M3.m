...
realTheta = [0 30 -45]; %Angle of sources, rad
...
eps_theta(:,1) = (-20:20)*1/10*sqrt(th_var(1));
eps_theta(:,2) = zeros(1,41);
eps_theta(:,3) = zeros(1,41);

for i = 1:length(eps_theta)
    tic
    [out.iy, out.iy0, ~, ~, ~, ~, ~, ~] = fMain(s, fs, M, N, realTheta, eps_theta(i, :));
    disp("gss: " + toc)
    ...
    stoi0(i, :) = out.stoi;
    tic
    [out.iy, out.iy0, ~, ~, ~, ~, ~, ~] = fGMain(s, fs, M, N, realTheta, th_var, eps_theta(i, :));
    disp("gss_g: " + toc)
    iyg(i, :, :) = out.iy;
...
save("tests\epsilon test\M3_" + char(datetime('now','Format','yyyy-MM-dd''T''HHmmss')));
