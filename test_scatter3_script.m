...
realTheta = [0 30 -40]; %Angle of sources, rad
...
eps_theta(:,1) = sqrt(th_var(1))*randn(L,1);
eps_theta(:,2) = sqrt(th_var(2))*randn(L,1);
eps_theta(:,3) = sqrt(th_var(3))*randn(L,1);
