...
realTheta = [30 40]; %Angle of sources, rad
...
i=1;
% for i = 2:length(th_var)
    for j = 1:length(th_var)
        eps_theta = [sqrt(th_var(1,i)) sqrt(th_var(2,j))];
        tic
...
save("tests\variance test\M2_eps_sigma_th_30_40_" + char(datetime('now','Format','yyyy-MM-dd''T''HHmmss')));